...
class AbstractExperience {
  ...
  #threejs_: THREE.WebGLRenderer | null = null;
  #camera_: THREE.PerspectiveCamera | null = null;
  #scene_: THREE.Scene | null = null;
  #clock_: THREE.Clock | null = null;
  #controls_: OrbitControls | null = null;

  #composer_: EffectComposer | null = null;
  #debugUI_: Pane | null = null;

  #ktx2Loader_: KTX2Loader | null = null;
  ...
  async #setupProject_(): Promise<void> {
    await this.#setupRenderer_();
    await this.#setupLoaders_();

    // Initialize post fx
    const postFXFolder = (this.#debugUI_ as any).addFolder({ title: 'PostFX', expanded: false });

    await this.#setupPostprocessing_(postFXFolder);

    // Initialize project
    const projectFolder = (this.#debugUI_ as any).addFolder({ title: 'Project', expanded: true });

    await this.onSetupProject(projectFolder);
  }

  async #setupLoaders_(): Promise<void> {
    this.#ktx2Loader_ = new KTX2Loader();
    this.#ktx2Loader_.setTranscoderPath('./libs/basis/');
    this.#ktx2Loader_.detectSupport(this.#threejs_!);
  }

  async #setupRenderer_(): Promise<void> {
    ...
    const sceneFolder = (this.#debugUI_ as any).addFolder({ title: 'Scene', expanded: false });
  ...
  createComposer(): EffectComposer {
    return new EffectComposer(this.#threejs_!);
  }

  createMainRenderPass(): RenderPass {
    return new RenderPass(this.#scene_!, this.#camera_!);
  }

  async #setupPostprocessing_(pane: any): Promise<void> {
    ...
    const lutPathToTexture: Record<string, any> = {};
    ...
    for (const key in luts) {
      ...
      loader.load(lutPath, (texture: any) => {
    ...
    const gtaoPass = new GTAOPass(this.#scene_!, this.#camera_!, window.innerWidth, window.innerHeight);
    ...
    gtaoFolder.addBinding(aoParameters, 'samples', { min: 1, max: 32, step: 1 }).on('change', (e: any) => {
      gtaoPass.updateGtaoMaterial( aoParameters );
    });
    gtaoFolder.addBinding(aoParameters, 'radius', { min: 0.0, max: 1.0 }).on('change', (e: any) => {
      gtaoPass.updateGtaoMaterial( aoParameters );
    });
    ...
    shaderFolder.addBinding(shaderOptions, 'intensity', { min: 0.0, max: 1.0 }).on('change', (e: any) => {
      vignettePass.material.uniforms.intensity.value = e.value;
    });
    shaderFolder.addBinding(shaderOptions, 'dropoff', { min: 0.0, max: 1.0 }).on('change', (e: any) => {
    ...
    colorCorrectionFolder.addBinding(colorCorrectionParams, 'brightness', { min: 0.0, max: 2.0 }).on('change', (e: any) => {
      brightnessContrastPass.material.uniforms.brightness.value = e.value;
    });
    colorCorrectionFolder.addBinding(colorCorrectionParams, 'contrast', { min: 0.0, max: 2.0 }).on('change', (e: any) => {
  ...
  async #loadKTX2_(path: string, srgb: boolean = true): Promise<THREE.Texture> {
    return new Promise((resolve, reject) => {
      this.#ktx2Loader_!.load(path, (texture: THREE.Texture) => {
        if (srgb) {
  ...
  async loadRGBE(path: string): Promise<void> {
    const rgbeLoader = new RGBELoader();

    return new Promise((resolve, reject) => {
      rgbeLoader.load(path , (hdrTexture: THREE.DataTexture) => {
  ...
  // Getters
  get Scene(): THREE.Scene | null { return this.#scene_; }
  get Camera(): THREE.PerspectiveCamera | null { return this.#camera_; }
  get Renderer(): THREE.WebGLRenderer | null { return this.#threejs_; }
}


export { AbstractExperience };