...
interface RenderTargetInfo {
  ...
  params: THREE.RenderTargetOptions;
  scale: number;
}

interface BloomRenderSettings {
  contrast: number;
  brightness: number;
  saturation: number;
  downRadius: number;
...
interface BloomCompositeSettings {
  mixFactor: number;
  strength: number;
}

interface BloomSetupSettings {
  levels: number;
}
...
const DOWNSAMPLE_SHADER: ShaderDefinition = {
  uniforms: {
...
const UPSAMPLE_SHADER: ShaderDefinition = {
  uniforms: {
    frameTexture: { value: null },
...
const COMPOSITE_SHADER: ShaderDefinition = {
  uniforms: {
    frameTexture: { value: null },
...
class BloomPassOptions {
  public render: BloomRenderSettings;
  public composite: BloomCompositeSettings;
  public setup: BloomSetupSettings;
...
class BloomPass extends Pass {
  ...
  #quad_: FullScreenQuad | null = null;
  #passes_: { [key: string]: THREE.ShaderMaterial } = {};
  #targets_: { [key: string]: RenderTargetInfo } = {};
  #settings_: BloomPassOptions;
  ...
  constructor(options?: BloomPassOptions) {
    ...
    this.#quad_ = new FullScreenQuad(undefined);

    this.#createRenderTargets_();
    this.#createPasses_();
  }

  get Settings(): BloomPassOptions {
    return this.#settings_;
  }

  #createRenderTarget_(name: string, scale: number, params: THREE.RenderTargetOptions): void {
    this.#targets_[name] = {
  ...
  #createRenderTargets_(): void {
    const bufferParams: THREE.RenderTargetOptions = {
  ...
  #createPass_(name: string, shaderData: ShaderDefinition): void {
  ...
  #renderPass_(name: string, renderer: THREE.WebGLRenderer, targetBuffer: THREE.WebGLRenderTarget | null): void {
    if (this.#quad_) {
  ...
  #buildColourMatrix_(contrast: number, brightness: number, saturation: number): THREE.Matrix4 {
